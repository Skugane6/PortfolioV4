...
export function maybeStartIdleAnimation(deps: CatAnimationDeps) {
  ...
  if (s.idleAnimation != null || s.idleTime < threshold) {
    return;
  }

  // A cat that only moves when it is nudged is meant to be found asleep, so it
  // curls up as soon as it settles rather than taking a detour through the other
  // idle animations, which can hold it awake for several seconds at a time.
  if (s.roamNudgeOnlyCfg && s.sleepEnabledCfg) {
