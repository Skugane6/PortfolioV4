...
/**
 * A line of text is a thin rect, so it needs a lower bar than a block element
 * before it counts as something to walk around.
 */
export const MIN_TEXT_OBSTACLE_AREA = 150;
/** Media that occupies space without containing any text of its own. */
export const MEDIA_OBSTACLE_SELECTOR = "img,svg,canvas,video,picture,input,textarea,select";
/** Oneko's own DOM, which must never become an obstacle to itself. */
export const ONEKO_OWN_DOM_SELECTOR = "#oneko-react,[data-oneko-ui]";
...
/** Half the sprite, so a walkable cell is one the whole cat fits in. */
export const SPRITE_RADIUS = TILE / 2;
