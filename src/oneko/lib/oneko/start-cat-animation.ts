...
export function startCatAnimation({
  ...
  // Nudging the cat is the whole interaction in roam nudge mode: it wakes up,
  // trots to another perch and settles again.
  const onCatPointerDown = (event: PointerEvent) => {
    const state = stateRef.current;
    if (!state.roamCfg) return;
    event.stopPropagation();
    state.roamWake = true;
    state.roamDwell = 0;
  };
  el.addEventListener("pointerdown", onCatPointerDown);
