...
export function updateZoneTarget(
  ...
  // Roam mode owns the target outright: no pointer tracking, no timed favorite
  // visits, just a walk to the next empty spot.
  if (s.roamCfg) {
    zones.targetId = null;
    zones.visitFrames = 0;
    const roamed = tickRoam(s, random);
    zones.movementTarget = zones.blocked.length
      ? (nearestSafePoint(roamed, zones.blocked, width, height, radius) ?? {
          x: s.nekoPosX,
          y: s.nekoPosY,
        })
      : roamed;
