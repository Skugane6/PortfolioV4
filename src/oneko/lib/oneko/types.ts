...
export interface OnekoProps {
  ...
  /**
   * Wander between random empty spots instead of tracking anything. Takes over
   * from followCursor when both are set, so the cat still moves with the pointer
   * ignored entirely. Default false.
   */
  roam?: boolean;
  /** CSS selector for the region the cat stays inside. It hides while that region is off screen. */
  roamArea?: string;
  /** Only move when clicked. Otherwise the cat naps at its perch indefinitely. Default false. */
  roamNudgeOnly?: boolean;
  /** Ticks to linger at each roam spot before moving on. Ignored in nudge mode. Default [180, 480]. */
  roamDwellFrames?: readonly [number, number];
  /** Shortest hop, as a fraction of the roam area's diagonal (0-1). Default 0.3. */
  roamMinHop?: number;
  /** Longest hop, as a fraction of the roam area's diagonal (0-1). Default 1. */
  roamMaxHop?: number;
...
export interface CatRuntimeState {
  ...
  roamCfg: boolean;
  roamDwellRange: readonly [number, number];
  roamMinHopCfg: number;
  roamMaxHopCfg: number;
  roamAreaCfg: string | null;
  roamNudgeOnlyCfg: boolean;
  roamTarget: PathPoint | null;
  roamDwell: number;
  roamWake: boolean;
  roamPivots: PathPoint[];
  roamPivotsSignature: string;
  roamHidden: boolean;
  roamStuckTicks: number;
  roamLastX: number;
  roamLastY: number;
